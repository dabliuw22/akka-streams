...
object MaterializeMainGraph extends App {
  ...
  val printSubscriber = Sink.foreach { println }
  ...
  val graphValue: Graph[SinkShape[String], Future[Int]] = GraphDSL
    ...
    implicit builder => (sinkCount, sinkPrint) =>
  ...
  def graphFlow[A, B](flow: Flow[A, B, _], sink: Sink[B, Future[B]]): Flow[A, B, Future[B]] =
    Flow.fromGraph(GraphDSL.create(sink) { implicit builder => s =>
      import GraphDSL.Implicits._
  ...
  val flow = graphFlow(Flow[String].filter { _.length >= 5 }.map { _.length }, countSubscriber)
